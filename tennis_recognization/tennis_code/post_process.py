...
def find_erode_marker(instance):
    area = instance.sum()
...
def mask_to_marker_by_erosion(mask):
    H,W = mask.shape[:2]
...
def mask_to_marker_by_enclose_circle(mask):
    H,W = mask.shape[:2]
    yy,xx= np.mgrid[:H, :W]
    num_instances = mask.max()
...
if __name__ == '__main__':
    ...
    if 0:  #experiment-1
        ...
        #distance = cv2.distanceTransform(binary, cv2.DIST_L2, maskSize=5)
        marker = mask_to_marker_by_enclose_circle(mask)
    ...
    water = watershed(-distance, marker, connectivity=1, mask=binary)


    #draw results
    color_overlay   = mask_to_color_overlay(water)
    color1_overlay  = mask_to_contour_overlay(water, color_overlay)
    contour_overlay = mask_to_contour_overlay(water, image, [0,0,255])
    contour_overlay = mask_to_contour_overlay(mask,  contour_overlay, [0,255,0])

    #marker and watershed transform
    distance = cv2.cvtColor(((distance/distance.max())*255).astype(np.uint8),cv2.COLOR_GRAY2BGR)
    marker   = (marker>0).astype(np.uint8)[..., np.newaxis]*np.array([0,0,255])
    marker_water = distance.copy()
    marker_water[np.where(marker[:,:,2]>0)] = [0,0,255]
    marker_water = mask_to_contour_overlay(water,  marker_water, [0,0,255])

    image_show('marker_water',np.hstack([marker_water, distance, contour_overlay]),resize=1)
