...
def filter_small(mask, threshold):
...
def run_npy_to_sumbit_csv():
    ...
    npy_dir   = '/root/share/project/kaggle/science2018/results/mask-se-resnext50-rcnn_2crop-mega-05d/ensemble/submit/masks'

    submit_dir  = \
        '/root/share/project/kaggle/science2018/results/mask-se-resnext50-rcnn_2crop-mega-05d/ensemble/submit'
    csv_file = submit_dir  + '/submission-color_gray.csv'
    ...
    for npy_file in npy_files:
        ...
        if 1:
            os.makedirs(npy_dir +'/overlays', exist_ok=True)

            #draw_shadow_text(overlay_mask, 'mask',  (5,15),0.5, (255,255,255), 1)

            #cv2.imwrite(out_dir +'/submit/npys/%s.png'%(name),color_overlay)
            cv2.imwrite(npy_dir +'/overlays/%s.png'%(name),all)

            #psd
            os.makedirs(npy_dir +'/overlays/%s'%name, exist_ok=True)
            cv2.imwrite(npy_dir +'/overlays/%s/%s.png'%(name,name),image)
            cv2.imwrite(npy_dir +'/overlays/%s/%s.mask.png'%(name,name),color_overlay)
            cv2.imwrite(npy_dir +'/overlays/%s/%s.contour.png'%(name,name),contour_overlay)
