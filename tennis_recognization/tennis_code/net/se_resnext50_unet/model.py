...
class UNet(nn.Module):
    ...
    def criterion(self, truth_foregrounds):
        cfg  = self.cfg
        ...
        weight = torch.ones(len(truth_foregrounds)).float().cuda()
        pos = truth_foregrounds == 1
        neg = truth_foregrounds != 1
        n_pos = len(truth_foregrounds[pos])
        n_neg = len(truth_foregrounds[neg])
        weight[pos] = 1.0
        if n_pos > 0 and n_neg is not 0:
            weight[neg] = float(n_pos) / n_neg
        
        probs = F.sigmoid(foregrounds)
        # self.foreground_loss = 0.5 * classify_loss(probs[pos], truth_foregrounds[pos]) + \
        #     0.5 * classify_loss(probs[neg], truth_foregrounds[neg])
        self.foreground_loss = F.binary_cross_entropy(probs, truth_foregrounds, 
            weight=weight, size_average=True)
        ...
        self.loss = self.foreground_loss
